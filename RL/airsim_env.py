...
class Env:
    ...
    def reset(self):
        ...
        self.dc.client.simSetCameraOrientation('4', airsim.to_quaternion(-30 * math.pi / 180, 0, 62 * math.pi / 180),
                                               vehicle_name=droneList[1])
        self.dc.client.simSetCameraOrientation('4', airsim.to_quaternion(-30 * math.pi / 180, 0, -62 * math.pi / 180),
                                               vehicle_name=droneList[2])
        time.sleep(1)
    ...
    def step(self, quad_offset_list):
        ...
        for droneidx in range(len(droneList[:-1])):
            self.dc.moveDrone(droneList[droneidx],
                              [quad_offset[droneidx][0], quad_offset[droneidx][1], quad_offset[droneidx][2]],
                              2 * timeslice)
        ...
        while time.time() - start_time < timeslice:
            ...
            for droneidx in range(len(droneList[:-1])):
                ...
                land = (quad_vel[droneidx].x_val == 0 and quad_vel[droneidx].y_val == 0 and quad_vel[
                    droneidx].z_val == 0)
        ...
        done = has_collided[droneidx] or self.gps_out_bounds(gps_dist) or any(
            status == 'dead' for status in img_reward.values())
    ...
    def calculate_bbox_zone(self, bbox, img):
        ...
        elif ((bcen_x > dbbox['xmin'] and bcen_x < nbbox['xmin']) or (
                bcen_x < dbbox['xmax'] and bcen_x > nbbox['xmax'])) and \
                ((bcen_y > dbbox['ymin'] and bcen_y < nbbox['ymin']) or (
                        bcen_y < dbbox['ymax'] and bcen_y > nbbox['ymax'])):
            status = 'normal'
        # Check if bbox center point is in slow zone
        elif ((bcen_x > nbbox['xmin'] and bcen_x < sbbox['xmin']) or (
                bcen_x > nbbox['xmax'] and bcen_x > sbbox['xmax'])) and \
                ((bcen_y > nbbox['ymin'] and bcen_y < sbbox['ymin']) or (
                        bcen_y < nbbox['ymax'] and bcen_y > sbbox['ymax'])):
            status = 'slow'
